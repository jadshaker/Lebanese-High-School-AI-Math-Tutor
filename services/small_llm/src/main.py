...
logger = StructuredLogger("small_llm")
...
def health_check() -> dict[str, str | bool]:
    ...
    try:
        req = Request(
            f"{Config.SMALL_LLM_SERVICE_URL}/v1/models",
            method="GET",
            headers={"Authorization": f"Bearer {Config.SMALL_LLM_API_KEY}"},
        )
        ...
        with urlopen(req, timeout=5) as response:
            ...
            model_available = any(
                model.get("id") == Config.SMALL_LLM_MODEL_NAME for model in models
            )
...
def chat_completions(
    ...
    try:
        ...
        model_name = request.model or Config.SMALL_LLM_MODEL_NAME
        logger.info(
