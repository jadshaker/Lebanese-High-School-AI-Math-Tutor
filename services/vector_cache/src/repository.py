...
class QdrantRepository:
    ...
    async def ensure_collections(self) -> None:
        ...
        if self.nodes_collection not in existing:
            ...
            # Create payload indexes for filtering by question and parent
            await self.client.create_payload_index(
                collection_name=self.nodes_collection,
    ...
    # Nodes store (user_input, system_response) pairs
    # Embedded by user_input for similarity matching
    ...
    async def add_interaction(
        ...
        """Add an interaction node (what user said + what system replied)"""
    ...
    async def search_children(
        ...
        Returns cache hit with matched node, or cache miss with parent_id for new node.
        """
        # Build filter: must match question_id AND parent_id
        conditions = [
        ...
        if parent_id:
            ...
            # Search for depth-1 nodes (direct children of question)
            # parent_id will be null for these
            conditions.append(
